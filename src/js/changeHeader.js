function PreviewImage(obj, imgPreviewId, divPreviewId) {
    ...
    var extention = obj.value.substring(obj.value.lastIndexOf(".") + 1).toLowerCase();//将文件的后缀名变为小写
    var browserVersion = window.navigator.userAgent.toUpperCase();//用来区分设备和不同的浏览器

    if (allowExtention.indexOf(extention) > -1) {//确认所选图片是否在allowExtention范围内
        if (browserVersion.indexOf("MSIE") > -1) {//MSIE是IE浏览器
